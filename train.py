...
def eval_classification(labels, logits):
    ...
    # Calculate the threshold using Youden's index
    spc = 1 - fpr
    j_scores = tpr - fpr
...
def train():
    ...
    parser.add_argument('--CV', type=int, default=5, help='the number of CV')
    ...
    for cv in range(args.CV):
        ...
        aupr_kfold = []
        acc_kfold = []
        drug1, drug2, cells, triples = load_gat.readRecData()  # Load Drug Combination Synergy Data
        entitys, relations, kgTriples = load_gat.readKGData()  # Load knowledge graph data
        kg_indexes = dataloader4KGNN.getKgIndexsFromKgTriples(kgTriples)  # Load KG triplet
        # Load entity and relation adjacency matrix
        ...
        for i in range(0, len(triples), split):
            ...
            if args.train_test_mode == 1:
                ...
                for e in range(args.n_epochs):
                    ...
                    for u1, u2, c, r in DataLoader(train_set, batch_size=args.batch_size,
                        ...
                        loss.backward()
                        optimizer.step()
                        # .item():Get the element value in the tensor
                        all_loss += loss.item()
                    ...
                    if (e == 0):
                        ...
                        torch.save(net.state_dict(),
                                   'model/{}_decoder{}.pkl'.format(now, count))  # Save the parameters in the network
                        print("save model")
                        earlystop_count = 0
            ...
            net1 = KGANS(args, max(entitys) + 1, max(relations) + 1,
                         args.e_dim, args.r_dim, adj_entity, adj_relation)
            net1.load_state_dict(torch.load('model/{}_decoder{}.pkl'.format(now, count)))  # save model
