...
def train():
    ...
    for cv in range(1):
        ...
        aupr_kfold = []
        acc_kfold = []
        drug1, drug2, cells, triples = load_gat.readRecData()
        entitys, relations, kgTriples = load_gat.readKGData()
        ...
        for i in range(0, len(triples), split):
            ...
            net1 = KGANS(args, max(entitys) + 1, max(relations) + 1,
                         args.e_dim, args.r_dim, adj_entity, adj_relation)
            net1.load_state_dict(torch.load('model/{}_decoder{}.pkl'.format(now, count)))
